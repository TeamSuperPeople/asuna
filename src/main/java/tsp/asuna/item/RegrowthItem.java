...
public class RegrowthItem extends Item {
    public RegrowthItem(Settings settings) {
        super(settings);
    }
    ...
    public static void createParticles(IWorld world, BlockPos pos, int count) {
        ...
        if (!blockState.isAir()) {
            for(int i = 0; i < count; ++i) {
                ...
                world.addParticle(ParticleTypes.HAPPY_VILLAGER, (double)((float)pos.getX() + RANDOM.nextFloat()), (double)pos.getY() + (double)RANDOM.nextFloat() * blockState.getOutlineShape(world, pos).getMaximum(Direction.Axis.Y), (double)((float)pos.getZ() + RANDOM.nextFloat()), d, e, f);
            }
