    ...
    public static void init() {
        CommandRegistry.INSTANCE.register(false, dispatcher -> {
            ...
            ArgumentCommandNode<ServerCommandSource, Identifier> altarRecipeID = CommandManager
                    ...
                    .suggests((context, builder) -> CommandSource.suggestIdentifiers(Asuna.ALTAR_RECIPE_MANAGER.getRecipes().keySet(), builder))
                    .executes(context -> {
                        Identifier id = IdentifierArgumentType.getIdentifier(context, "id");
                        ServerPlayerEntity playerEntity = context.getSource().getPlayer();
                        AltarRecipe recipe = Asuna.ALTAR_RECIPE_MANAGER.getRecipes().get(id);
